...
assert srv._ADMIN_AUTH_CONFIGURED is False, (
    "should be unconfigured with no password/secret set"
)
...
real_sig = hmac.new(
    srv._ADMIN_COOKIE_SECRET.encode(), expiry.encode(), hashlib.sha256
).hexdigest()
own_cookie = f"{expiry}.{real_sig}"
result2 = srv._admin_cookie_valid(own_cookie)
assert result2 is False, (
    "should still fail closed when unconfigured, even with a technically-valid HMAC"
)

print(
    "PASS: admin cookie forgery is blocked when ADMIN_PASSWORD/ADMIN_COOKIE_SECRET are unset"
)
...
assert srv._admin_cookie_valid(good_cookie) is True, (
    "legit cookie should validate when configured"
)
