...
```
Claude (claude.ai)
      │  HTTPS /sse + /messages (or /mcp)
      ▼
Render / Cloud (codespaces-mcp — Official @modelcontextprotocol/sdk)
      │
...
**Key facts:**
- Official `@modelcontextprotocol/sdk` implementation using `Server` and `SSEServerTransport`.
- Endpoints:
  - `GET /sse`: Server-Sent Events stream for MCP client connections.
  - `POST /messages`: JSON-RPC message delivery endpoint.
  - `POST /mcp`: Direct JSON-RPC 2.0 endpoint for standard HTTP clients.
- Protected by `MCP_SERVER_PASSWORD` via HTTP `Authorization: Bearer <token>` or `X-MCP-Token`.
- Strict headless MCP server: Zero web frontend templates or admin HTML bloat.
