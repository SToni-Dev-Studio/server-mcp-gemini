...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  button { background: #0284c7; border: none; font-weight: 600; cursor: pointer; transition: background 0.2s; color: #fff; }
...
<div class="container">
  <header>
    <div>
      ...
      <p style="color:var(--muted); font-size:0.85rem;">Built-in Executive & Secrets Controller (.exe)</p>
    </div>
  ...
  <div class="grid">
    <div class="card">
      ...
      <p><strong>Loopback Port:</strong> <span id="st-port">7842</span></p>
    ...
    <div class="card">
      ...
      <p style="font-size:0.8rem; color:var(--muted);">Setting a secret requires Claude / MCP requests to provide the matching X-Organiser-Secret header.</p>
    </div>
  </div>
...
async function fetchStatus() {
  try {
    ...
    document.getElementById('st-plat').textContent = 'Error';
  }
}
...
async function updateSecret() {
  ...
  try {
    ...
    if(res.ok) {
      ...
      msg.textContent=d.message || 'Secret updated successfully!';
      document.getElementById('new-sec').value='';
      fetchStatus();
    } else {
      msg.className='msg err';
      msg.textContent=d.error || 'Failed to update secret';
    }
  } catch(e) {
    msg.className='msg err';
    msg.textContent='Error updating secret: ' + e;
  }
}
...
static Response h_dashboard(const Request&) {
    Response r;
    r.status = 200;
...
static Response h_set_secret(const Request& req) {
    std::string secret = json_str(req.body, "secret");
    if (secret.empty()) {
        return Response::err(400, "Secret cannot be empty");
    }
    g_secret = secret;
    std::ofstream sf(".secrets");
    if (sf) {
        sf << "ORGANISER_SECRET=" << secret << "\n";
    }
    return Response::ok(Json::obj({
        {"success", Json::boolean(true)},
        {"message", Json::str("Secret updated successfully")},
    }));
}
...
static Response h_status(const Request&) {
    return Response::ok(Json::obj({
        ...
        {"has_secret",      Json::boolean(!g_secret.empty())},
        {"watched_folders", "[]"},
    }));
}
...
static void handle_conn(SOCKET sock) {
    ...
    // Auth check
    if (!g_secret.empty()) {
        bool is_local_dashboard = (req.path == "/" || req.path == "/dashboard" || req.path == "/set_secret");
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            std::cout << "========================================\n";
            std::cout << "  Organiser Agent v" << VERSION << " (.exe)\n";
            std::cout << "========================================\n\n";
            std::cout << "Usage:\n";
            std::cout << "  organiser-agent.exe [options]\n\n";
            std::cout << "Options:\n";
            std::cout << "  --status              Print local agent status and exit\n";
            std::cout << "  --set-secret <value>  Update ORGANISER_SECRET in memory & save to .secrets\n";
            std::cout << "  --port <number>       Specify listen port (default: 7842)\n";
            std::cout << "  --help, -h            Show this help message\n";
            return 0;
        } else if (arg == "--status") {
            std::cout << "Platform : " << platform_name() << "\n";
            std::cout << "Version  : " << VERSION << "\n";
            std::cout << "Auth     : " << (g_secret.empty() ? "No secret (open)" : "Secret active") << "\n";
            std::cout << "Port     : " << g_port << "\n";
            return 0;
        } else if (arg == "--set-secret" && i + 1 < argc) {
            std::string sec = argv[++i];
            g_secret = sec;
            std::ofstream sf(".secrets");
            if (sf) sf << "ORGANISER_SECRET=" << sec << "\n";
            std::cout << "Secret updated to: " << sec << " (saved to .secrets)\n";
            return 0;
    ...
    // Register routes
    add_route("GET",  "/",             h_dashboard);
    add_route("GET",  "/dashboard",    h_dashboard);
    add_route("POST", "/set_secret",   h_set_secret);
